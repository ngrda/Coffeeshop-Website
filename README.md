...
├── images/
│   ├── logo.png
│   ├── coffee.mp4        # Hero background video
│   └── hero-poster.jpg   # Static fallback shown while the video loads or if it fails
└── README.md
```

> **Note:** `hero-poster.jpg` is referenced as the video's `poster` (and as the fallback for slow connections / data-saver mode) — make sure it's added to `images/` or the hero will show a blank background when the video can't load.
